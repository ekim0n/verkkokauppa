...
app.get("/categories", async (req, res) => {
  try {
    ...
    const [rows] = await connection.execute(
      "SELECT category_name categoryName, category_description description FROM product_category"
    );
...
app.get("/customer", async (req, res) => {
  ...
  try {
    ...
    const connection = await mysql.createConnection(conf);
    const [rows] = await connection.execute(
      "SELECT first_name fname, last_name lname, username, is_admin FROM customer WHERE username=?",
      [username]
    );
...
app.post("/categories", async (req, res) => {
  ...
  try {
    ...
    for (const category of categories) {
      await connection.execute("INSERT INTO product_category VALUES (?,?)", [
  ...
  try {
    ...
    for (const product of products) {
      await connection.execute(
        "INSERT INTO product (product_name, product_description, product_description1, product_description2, product_description3, price, price_usd, image_url,category) VALUES (?,?,?,?,?,?,?,?,?)",
        [
          product.productName,
  ...
  try {
    ...
    for (const product of order.products) {
      await connection.execute(
        ...
        [orderId, product.id, product.count]
      );
    }
...
app.post("/palaute", upload.none(), async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.execute(
      "INSERT INTO palaute(feedback, tuote) VALUES(?,?)",
      [feedback, tuote]
    );
...
app.post("/changepassword", upload.none(), async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.execute(
      "SELECT pw FROM customer WHERE username=?",
      [username]
    );
    ...
    await connection.execute("UPDATE customer SET pw=? WHERE username=?", [
      newPasswordHash,
      username,
    ]);
...
app.get("/users", async (req, res) => {
  try {
    ...
    const [rows] = await connection.execute(
      "SELECT first_name, last_name, username FROM customer"
    );
...
async function getOrders(username) {
  try {
    const connection = await mysql.createConnection(conf);
    const [rows] = await connection.execute(
      "SELECT customer_order.order_date AS date, customer_order.id as orderId FROM customer_order INNER JOIN customer ON customer.id = customer_order.customer_id WHERE customer.username=?",
      [username]
    );
    ...
    for (const row of rows) {
      const [products] = await connection.execute(
        "SELECT id,product_name productName,price,image_url imageUrl, category, quantity  FROM product INNER JOIN order_line ON order_line.product_id = product.id WHERE order_line.order_id=?",
        [row.orderId]
      );
...
app.post("/customerfeedback", async (req, res) => {
  ...
  try {
    const connection = await mysql.createConnection(conf);
    await connection.execute(
      "INSERT INTO customerfeedback (email, nickname, feedback, rating) VALUES (?, ?, ?, ?)",
      [email, nickname, feedback, rating]
    );
    connection.end();
